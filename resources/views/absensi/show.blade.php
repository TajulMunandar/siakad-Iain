        ...
        <div class="card col-sm-6 col-md-12">
          <div class="card-body">
            <table id="myTable"
              ...
              <tbody>
                @foreach ($absensi->kelas->mahasiswa as $mahasiswa)
                  {{-- {{ dd($mahasiswa->absensiDetail->first()->id_status) }} --}}
                  <tr>
                      ...
                      @foreach ($statusAbsensis as $status)
                        <div class="form-check form-check-inline">
                          <input class="form-check-input" type="radio"
                            name="statusRadio[{{ $mahasiswa->id }}]"
                            id="status{{ $status->id }}_{{ $mahasiswa->id }}"
                            value="{{ $status->id }}"
                            {{ optional($mahasiswa->absensiDetail->where('pertemuan', $pertemuan)->first())->id_status == $status->id ? 'checked' : '' }}>
                          <label class="form-check-label"
                            for="status{{ $status->id }}_{{ $mahasiswa->id }}">
                            {{ $status->status }}
                          </label>
                        </div>
                      @endforeach
                    ...
                    <td>
                      <input type="text" class="form-control"
                        name="keterangan[{{ $mahasiswa->id }}]" id="keterangan{{ $mahasiswa->id }}"
                        value="{{ optional($mahasiswa->absensiDetail->where('pertemuan', $pertemuan)->first())->keterangan }}" placeholder="Keterangan">
                    </td>
                  </tr>
                @endforeach
              </tbody>
